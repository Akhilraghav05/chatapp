...
export class ChatBoxComponent implements OnInit {
  ...
  public getOnlineUserList: any = () => {
    this.SocketService.onlineUserList().subscribe((userList: { [x: string]: any; }) => {
      this.userList = [];
  ...
  public getPreviousChatWithAUser: any = () => {
    let previousData =
      this.messageList.length > 0 ? this.messageList.slice() : []; //making messagelist empty
  ...
  public userSelectedToChat: any = (id: string, name: string) => {
    ...
    this.userList.map((user: { userId: string; chatting: boolean; }) => {
      if (user.userId == id) {
        user.chatting = true;
      } else {
        user.chatting = false;
  ...
  public showUserName = (name: string) => {
    console.log(this.toastr.success("You are chatting with " + name));
  };
}
